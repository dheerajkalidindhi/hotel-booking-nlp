...
def run_query(intent, entities=None):
    ...
    if intent == "top_user":
        ...
        query = """
            SELECT TO_CHAR(booking_date, 'Day') AS day_name, COUNT(*) AS total
            FROM bookings
            GROUP BY day_name
            ORDER BY total DESC
            LIMIT 1;
        ...
        query = """
            SELECT city, ROUND(AVG(rating), 2) AS avg_rating, COUNT(*) AS total
            FROM bookings
            GROUP BY city
        ...
        query = """
            SELECT hotel_name, ROUND(AVG(rating), 2) AS avg_rating, COUNT(*) AS total
            FROM bookings
            GROUP BY hotel_name
            ORDER BY avg_rating DESC;
        """
        ...
        query = """
            SELECT *, (check_out_date - check_in_date) AS stay_days
            FROM bookings
        ...
        query = """
            SELECT TO_CHAR(booking_date, 'YYYY-MM') AS month, COUNT(*) AS total
            FROM bookings
            GROUP BY month
            ORDER BY month DESC;
        """
        ...
        query = """
            SELECT TO_CHAR(booking_date, 'Month') AS month, COUNT(*) AS total
            FROM bookings
            GROUP BY month
            ORDER BY total DESC
            LIMIT 1;
